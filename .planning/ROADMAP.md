...
- [ ] 04-08-PLAN.md — GdprExportService (REPEATABLE READ READ ONLY transaction — D-17 / Pattern 3 / Pitfall §5/§7) + GET /api/players/{id}/export (player JWT sub-claim match) + GET /admin/api/players/{id}/export (Superadmin + audit row) + 25 MB cap (D-18) + GdprExportContractTests (SC#5) + RankAdjustService (SERIALIZABLE tx — UPDATE player_ranks + IAdminAuditWriter atomic per RANK-12) + POST /admin/api/players/{id}/rank-adjust + RankAdjustDialog.razor + MainLayout switch arm + AdminRankAdjustTransactionTests (SC#6 — rollback symmetry on faulty audit writer) + samples/TicTacToeDuel full Phase 1-4 boot integration
**UI hint**: no
