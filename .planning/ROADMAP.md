...
**Plans**: 10 plans
- [ ] 05-01-PLAN.md — Wave 0 scaffolding: three test projects (Unit/Integration/LoadTests), MatchmakingCollection fixture, MatchmakingTestModelCustomizer (Pitfall §3), StepClock helper, MatchmakingAdvisoryLockKeyTests (Wave 0 mandatory; intentionally RED on Test A until 05-02 lands the live-verified key)
- [ ] 05-02-PLAN.md — Five entities (Party/PartyMember/MatchmakingTicket/TicketEvent/DeclineHistory) + EF configurations (integer-enum storage, no HasConversion<string>) + MatchmakingModelBuilderExtension + MatchmakingMigrationConstants (live-verified hashtext) + MatchmakingDesignTimeDbContextFactory (excludes all prior-package entities) + MatchmakingMigrationHostedService + 20260516000000_MatchmakingInitial migration (CITEXT party_code reuses Phase 2 extension — Pitfall §9) + MatchmakingMigrationDeterminismTests + BLOCKING migration-boundary checkpoint
- [ ] 05-03-PLAN.md — GameKitMatchmakingOptions root + 4 nested sub-options (Ticker/Cooldown/Analytics/Reconciler) + IValidateOptions guards + MatchmakingLadderConfig (D-11..D-14 defaults) + IGameKitMatchmakingBuilder + AddMatchmaking + AddLadder fluent extensions + MatchmakingRedisKeys constants + MapMatchmaking() stub
- [ ] 05-04-PLAN.md — IMatchmakingStrategy contract + EloRangeMatchmakingStrategy (bracket flex per D-11 + per-aggregator switch per D-13) + PartyRatingAggregatorService (Mean/Max/GlickoWeighted) + AtomicClaimScript (Lua ≤30 lines, fencing-token first; Pitfall §2) + IPartyService/PartyService (SERIALIZABLE single-active-party per RESEARCH §OQ-2) + PartyCodeGenerator (RandomNumberGenerator + Crockford base32 6-char) + unit & integration tests
- [ ] 05-05-PLAN.md — MatchmakerLeaseHelper (Polly v8 mirror of Rankings; new lock key gamekit:matchmaking:matcher:lock) + MatchmakerTickerService (BackgroundService + PeriodicTimer 500ms; leader-only; renew-before-each-pool per Pitfall §2; proposal-sweep step per Pitfall §10) + ProposalSweeper + MatchmakingActivitySource OTel + MatchmakingLeaderElectionTests (SC#4 phase gate: 2-replica + forced failover) + ProposalSweepTests
- [ ] 05-06-PLAN.md — IProposalService (Accept/Decline with atomic SADD+SCARD complete check via Lua) + DeclineCooldownService (D-08 escalating 3/15/30 min) + TeamAssignmentService (v1 random per CONTEXT discretion) + ProposalAcceptHappyPathTests + ProposalDeclineRequeueTests (verifies D-09 queuedAt preservation) + CooldownEnforcementTests
- [ ] 05-07-PLAN.md — MatchmakingReconcilerService (30s + startup; leader-only; NEVER writes to Redis — Pitfall §1 the most important rule) + MatchmakingAnalyticsDrainService (Channel<TicketEvent> bounded drop-newest per D-15; Polly v8 retry per D-16) + MatchmakingRetentionCleanupService (nightly 30-day per D-17) + MatchmakingMeter (matchmaking.analytics.dropped_events OTel counter per Pitfall §7) + Channel/Writer/Reader singleton wiring + integration tests
- [ ] 05-08-PLAN.md — Player-facing HTTP surface: PartyEndpoints (4 routes) + MatchmakingEndpoints (5 routes incl. long-poll with linked HttpContext.RequestAborted CTS per Pitfall §5) + LongPollStatusHandler + MatchmakingRateLimitRegistrations (gamekit:mm:enqueue 5/min/player + new gamekit:mm:party_join per-IP for T-05-08-04) + ValidationEndpointFilter + IMatchmakingObservability/RedisMatchmakingObservability + MatchmakingAdminEndpoints (pause-queue + drain-queue with Superadmin+antiforgery+audit) + Admin.UI ProjectReference + AdminAuditActions/AuditSentenceTemplates/AdminCommandRegistry additions + QueueDepth.razor fill-in (replaces Phase 3 placeholder) + MatchmakingTestApp + SC#1/SC#5/SC#6 integration tests + PartyEndpointTests + LongPollStatusTests (Pitfall §5 abort)
- [ ] 05-09-PLAN.md — IChaosInterceptor seam + NullChaosInterceptor production default + AbortingChaosInterceptor test double + probe insertions in MatchmakerTickerService (BeforeLuaClaim) and ProposalService (BeforeSessionInsert) + MatchmakingChaosTests (SC#2 phase gate; 100 parties + chaos abort + reconciler sweep + 4 invariants) + TicTacToeDuel sample wiring (AddMatchmaking + AddLadder + matchmaking.html 1v1-only UI per CONTEXT.md) + human-verify UAT + OQ-1..OQ-6 resolution in SUMMARY
...
| 5. Matchmaking + Parties | 0/10 | Not started | - |
