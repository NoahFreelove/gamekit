...
**Plans**: 8 plans
- [x] 04-01-PLAN.md — Wave 0: test csprojs (GameKit.Rankings.Tests + GameKit.Rankings.Integration.Tests) + RankingsFixture composite (PostgresFixture + RedisFixture) + Glickman worked-example JSON fixture + BLOCKING human-verify Glicko-2 BSD/MIT license variant (Pitfall §1 close) + THIRD-PARTY-NOTICES.md + REUSE.toml update — **COMPLETE** (BSD-3-Clause confirmed; commits 83ca444+d7bd6d5+65054c6)
- [x] 04-02-PLAN.md — Entities (Ladder, PlayerRank, LadderSeason, SeasonRankArchive, ServiceToken, PendingRatingUpdate, SessionCompleteIdempotency, SeasonResetPolicy enum) + 7 EF configurations + RankingsModelBuilderExtension + RankingsDesignTimeDbContextFactory + RankingsMigrationHostedService + 20260515000000_RankingsInitial migration + raw-SQL FK fk_game_sessions_ladders + RankingsAdvisoryLockKeyTests (live-verify hashtext) + SchemaTypeAssertions (SC#3 — double precision)
- [x] 04-03-PLAN.md — Vendored Glicko-2 source (Rating, RatingCalculator, RatingPeriodResults, Result — under GameKit.Rankings.Glicko2 internal namespace with verified upstream attribution) + IRankingAlgorithm.Apply(state, batch) interface (RANK-04 batched-only) + Glicko2Algorithm adapter with tau:0.5 (Pitfall §2) + Glicko2WorkedExampleTests (RANK-05) + Glicko2AlgorithmContractTests
- [x] 04-04-PLAN.md — GameKitRankingsOptions + IGameKitRankingsBuilder + AddRankings/AddLadder fluent extensions + StartupLadderUpserter (IHostedService) + ServiceTokenAuthenticationHandler (Pattern 2, custom AuthenticationHandler against SHA-256-hashed service_tokens) + ServiceTokenAuthorizationPolicy + dotnet gamekit service-token {issue|revoke|list} CLI verbs + RankingsCliModelCustomizer (Pitfall §3 defense)
- [x] 04-05-PLAN.md — IPostSessionCompleteHandler port (defined in Core per D-22) + SessionEndpoints in GameKit.Core/Http + ISessionCompleteService + CanonicalJsonHasher (Open Q5 — sorted-participant STJ canonicalization) + IdempotencyKeyEndpointFilter + ValidationEndpointFilter (Rankings copy per Open Q4) + RankingsRateLimitRegistrations (gamekit:sessions:complete 300/min/token) + PendingRatingUpdatesAdapter + SessionCompleteIdempotencyTests (SC#2 — 5× retry produces exactly one delta)
- [x] 04-06-PLAN.md — RankingsTickerService (BackgroundService + PeriodicTimer + Polly v8 retry + Redis IDatabase.LockTake distributed lock — D-03) + RankingsTickerLeaseHelper + IRankingsTicker (testable inner loop) + IdempotencyCleanupService (24h TTL cleanup of session_complete_idempotency) + RankingsTickerLeaderElectionTests + LazyRankCreationTests (RANK-07) + Glicko2ConvergenceTests (SC#1 — 1000-match two-population convergence)
- [x] 04-07-PLAN.md — ILeaderboardService.TopAsync + AroundAsync (RANK-08 / D-23) + IEndSeasonService.EndAsync (SERIALIZABLE tx — close ladder_seasons + open new + archive into season_rank_archive + apply SeasonResetPolicy {SoftRegress|HardReset|ArchiveOnly} + write LadderEndSeason audit row) + POST /admin/api/ladders/{id}/end-season + EndSeasonDialog.razor (type-name-to-confirm) + MainLayout switch arm + AdminCommandRegistry/AdminAuditActions/AuditSentenceTemplates additions + SeasonArchiveLeaderboardTests (SC#4)
- [x] 04-08-PLAN.md — GdprExportService (REPEATABLE READ READ ONLY transaction — D-17 / Pattern 3 / Pitfall §5/§7) + GET /api/players/{id}/export (player JWT sub-claim match) + GET /admin/api/players/{id}/export (Superadmin + audit row) + 25 MB cap (D-18) + GdprExportContractTests (SC#5) + RankAdjustService (SERIALIZABLE tx — UPDATE player_ranks + IAdminAuditWriter atomic per RANK-12) + POST /admin/api/players/{id}/rank-adjust + RankAdjustDialog.razor + MainLayout switch arm + AdminRankAdjustTransactionTests (SC#6 — rollback symmetry on faulty audit writer) + samples/TicTacToeDuel full Phase 1-4 boot integration
**UI hint**: no
...
**Plans**: 10 plans
- [x] 05-01-PLAN.md — Wave 0 scaffolding: three test projects (Unit/Integration/LoadTests), MatchmakingCollection fixture, MatchmakingTestModelCustomizer (Pitfall §3), StepClock helper, MatchmakingAdvisoryLockKeyTests (Wave 0 mandatory; intentionally RED on Test A until 05-02 lands the live-verified key) [2026-05-17]
- [x] 05-02-PLAN.md — Five entities (Party/PartyMember/MatchmakingTicket/TicketEvent/DeclineHistory) + EF configurations (integer-enum storage, no HasConversion<string>) + MatchmakingModelBuilderExtension + MatchmakingMigrationConstants (live-verified AdvisoryLockKey=388956820L) + MatchmakingDesignTimeDbContextFactory (15-type prior-package ExcludeFromMigrations enumeration) + MatchmakingMigrationHostedService + 20260516000000_MatchmakingInitial migration (CITEXT party_code reuses Phase 2 extension — Pitfall §9; FK principalTable=`ladders` hand-corrected) + MatchmakingMigrationDeterminismTests + BLOCKING migration-boundary checkpoint operator-approved 2026-05-17 [2026-05-17]
- [x] 05-03-PLAN.md — GameKitMatchmakingOptions root + 4 nested sub-options (Ticker/Cooldown/Analytics/Reconciler) + IValidateOptions guards + MatchmakingLadderConfig (D-11..D-14 defaults) + IGameKitMatchmakingBuilder + AddMatchmaking + AddLadder fluent extensions + MatchmakingRedisKeys constants + MapMatchmaking() stub — **COMPLETE** (commits fd34a72 + bb2d7f6 + cdbaaae; 23 new unit tests; Scrutor scan + endpoint mapping deferred to 05-04/05-08 with TODO markers)
- [x] 05-04-PLAN.md — IMatchmakingStrategy contract + EloRangeMatchmakingStrategy (bracket flex per D-11 + per-aggregator switch per D-13) + PartyRatingAggregatorService (Mean/Max/GlickoWeighted) + AtomicClaimScript (Lua ≤30 lines, fencing-token first; Pitfall §2) + IPartyService/PartyService (SERIALIZABLE single-active-party per RESEARCH §OQ-2) + PartyCodeGenerator (RandomNumberGenerator + Crockford base32 6-char) + unit & integration tests
- [x] 05-05-PLAN.md — MatchmakerLeaseHelper (Polly v8 mirror of Rankings; new lock key gamekit:matchmaking:matcher:lock) + MatchmakerTickerService (BackgroundService + PeriodicTimer 500ms; leader-only; renew-before-each-pool per Pitfall §2; proposal-sweep step per Pitfall §10) + ProposalSweeper + MatchmakingActivitySource OTel + MatchmakingLeaderElectionTests (SC#4 phase gate: 2-replica + forced failover) + ProposalSweepTests
- [x] 05-06-PLAN.md — IProposalService (Accept/Decline with atomic SADD+SCARD complete check via Lua) + DeclineCooldownService (D-08 escalating 3/15/30 min) + TeamAssignmentService (v1 random per CONTEXT discretion) + ProposalAcceptHappyPathTests + ProposalDeclineRequeueTests (verifies D-09 queuedAt preservation) + CooldownEnforcementTests
...
**Plans**: 10 plans
- [ ] 06-01-PLAN.md — Wave 0 foundation: ROADMAP SC#1+SC#5 fixes, Microsoft.AspNetCore.OpenApi 10.0.8 CPM pin, Directory.Build.props CompilerVisibleProperty + GameKit.targets import, GameKit.Build Roslyn source generator + GameKit.OpenApi skeleton + Analyzer ProjectRef wired into all 7 src csprojs
- [ ] 06-02-PLAN.md — Core ports: ISessionLifecycleObserver, ISessionStartService/ISessionAbandonService contracts + GameKitVersionMismatchException + GameKitVersionAssertionHostedService registered AT INDEX 0 of services in AddGameKit()
- [ ] 06-03-PLAN.md — Wave 0 test scaffolding: 4 new test csprojs (Presence.Tests, Presence.Integration.Tests, OpenApi.Integration.Tests, Distribution.Integration.Tests) + CollectionDefinitions + DistributionIntegrationFixture composing the already-3-role PostgresFixture
- [ ] 06-04-PLAN.md — Presence runtime: options + validator + Redis-keyed presence provider with atomic Lua precedence (PATTERNS warning #6), IPresenceWriter, PresenceSessionObserver, AddPresence + UsePresence + MapPresence, heartbeat endpoint, sample TicTacToeDuel wiring
- [ ] 06-05-PLAN.md — Sessions /start + /abandon endpoints + SessionStartService/SessionAbandonService implementations + SessionCompleteService modified to fan-out ISessionLifecycleObserver alongside IPostSessionCompleteHandler (D-21 backwards-compat) + end-to-end PresenceSessionObserver wire-up validation
- [ ] 06-06-PLAN.md — GameKit.OpenApi 7th package runtime: GameKitOpenApiOptions + GameKitBearerSchemeTransformer + GameKitInfoTransformer + AddGameKitOpenApi with INLINE ShouldInclude admin filter (D-19, PATTERNS warning #1) + MapGameKitOpenApi + sample wiring + 3 contract tests (Coverage, Bearer, AdminExclusion)
- [ ] 06-07-PLAN.md — Admin UI Presence panel (PresencePanel.razor + code-behind + 10s polling + MissingPackageAlert short-circuit + StatusChip precedence fix for InMatch/Offline + CSS chip modifiers + SideNav row) + substring-contract integration tests + human-verify checkpoint
- [ ] 06-08-PLAN.md — TicTacToeDuel.GameServer console app (gamekit_reader topology) + DIST-02 reader-denied-INSERT empirical test (SQLSTATE 42501) + OPS-04 version-stamped-across-7-packages test + OPS-05 mismatch assertion test + OPS-06 clean-install migration test
- [ ] 06-09-PLAN.md — GameKit.Templates NuGet template package + DIST-03 cross-process smoke test + DIST-04 template package shape test
- [ ] 06-10-PLAN.md — docs/ops/ 9-file production-readiness guide + README "Production Deployment" link + DIST-06 CS1591 audit + end-of-phase human-verify checkpoint (template + docs together)
**UI hint**: yes (Plan 06-07 — Admin Presence panel)
...
| 5. Matchmaking + Parties | 10/10 | Complete    | 2026-05-25 |
| 6. Presence + OpenAPI + Distribution | 0/10 | In progress (planned) | - |
