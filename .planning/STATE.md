...
| Phase 05 P02 | 8min | 4 tasks | 22 files |
| Phase 05 P03 | 7min | 3 tasks | 17 files |
| Phase 05 P04 | 21min | 3 tasks | 23 files |
| Phase 05 P05 | 22min | 3 tasks | 11 files |
| Phase 05 P06 | 13min | 3 tasks | 14 files |
...
| Phase 09 P01 | 7 | 3 tasks | 13 files |
...
| GameKit.Rankings InternalsVisibleTo grants extended with GameKit.Matchmaking.Integration.Tests — MatchmakingTestModelCustomizer calls new RankingsModelBuilderExtension().ApplyTo(modelBuilder) for cross-package test models (EloRangeMatchmakingStrategy reads player_ranks); mirrors GameKit.Auth → GameKit.Admin.Integration.Tests grant pattern from Plan 03-06 | 05-01 execution |
| Wave 0 Integration.Tests build state is RED-by-design: MatchmakingTestModelCustomizer.cs + MatchmakingAdvisoryLockKeyTests.cs reference GameKit.Matchmaking.Data.{MatchmakingMigrationConstants,MatchmakingModelBuilderExtension} which land in Plan 05-02. Single CS0234 error is the deterministic gate for Plan 05-02; build flips green after 05-02 ships the data layer | 05-01 execution |
...
| MatchmakingMigrationConstants XML doc references `GameKit.Core.Data.GameKitMigrationConstants` (NOT `CoreMigrationConstants`) — the Core constants class is named `GameKitMigrationConstants` (Phase 1 naming convention). Initial Rankings-template copy used wrong cref; corrected to avoid CS1574 doc warning | 05-02 execution |
| CITEXT on party_code reuses Phase 2 Auth's `CREATE EXTENSION IF NOT EXISTS citext` — Matchmaking migration contains zero `CREATE EXTENSION` calls, only the no-op `Npgsql:PostgresExtension:citext` annotation; `HasColumnType("citext")` override applied directly in the migration (EF does not infer citext from a string property). Pitfall §9 closed at the SQL level rather than application code | 05-02 execution |
| GameKit.Matchmaking.csproj gains EF Core / Npgsql / EFCore.Design package references + FrameworkReference Microsoft.AspNetCore.App — the Phase-1 skeleton csproj had only the Core ProjectReference; Plan 05-02 brings it up to parity with GameKit.Rankings.csproj from Plan 04-02. All versions resolved via CPM from Directory.Packages.props (zero new pins) | 05-02 execution |
| GameKitMatchmakingOptions defaults pinned verbatim from CONTEXT D-07/D-08/D-15/D-17 and RESEARCH §Decision 6/7/10/13: AcceptTimeoutSeconds=10, TicketRetentionDays=30, MatchmakingEnqueueRatePerMinute=5, Ticker.{TickIntervalMs=500, LockTtlSeconds=90, LockKey="gamekit:matchmaking:matcher:lock", MaxIterationBudgetMs=50}, Cooldown.{WindowMinutes=60, Step1=3, Step2=15, Step3=30}, Analytics.{ChannelCapacity=10000, DrainBatchSize=100, DrainIntervalSeconds=5, PollyMaxRetryAttempts=4, PollyBaseDelayMs=500, PollyTimeoutSeconds=30}, Reconciler.{SweepIntervalSeconds=30, StaleTicketThresholdMinutes=5, OrphanSessionThresholdMinutes=10, LeaderOnly=true} | 05-03 execution |
| MatchmakingRedisKeys.MatcherLock literal = "gamekit:matchmaking:matcher:lock" — duplicated at GameKitMatchmakingTickerOptions.LockKey default (defense-in-depth: operator overrides MUST update both consistently; matchmaker resolves from options at runtime, but ad-hoc tooling reads the constant directly) | 05-03 execution |
| MatchmakingOptionsValidator implements IValidateOptions<GameKitMatchmakingOptions> + AddMatchmaking calls ValidateOnStart() — host startup fails fast on any invalid option (mitigates T-05-03-01); per-ladder invariants (BracketRampSeconds>0, BracketEnd>=BracketStart, MaxPartyRatingSpread null or >0) enforced at GameKitMatchmakingBuilder.AddLadder registration time (the ladder list is held by the builder, not IOptions). Mirrors Phase 4 builder-validation precedent | 05-03 execution |
| MatchmakingLadderConfig.Name is the case-insensitive JOIN KEY against the Rankings-owned ladder of the same name (a consumer calling AddRankings().AddLadder("main", ...) + AddMatchmaking().AddLadder("main", ...) configures both surfaces for one logical ladder); case-insensitive dedup at both builder layers (HashSet<string>(OrdinalIgnoreCase)) enforces the convention | 05-03 execution |
| PartyRatingAggregator integer enum (Mean=0, Max=1, GlickoWeighted=2) — Phase 5 mandatory integer-enum pattern (no HasConversion<string>()); default Mean per CONTEXT D-13 | 05-03 execution |
...
| Open Question | Phase | Expected Resolution |
|---------------|-------|---------------------|
...
| 260712-t3r | Add admin-console screenshot showcase to marketing site: 4 live-captured WebP shots (players master-detail, audit log, queue depth w/ leader lease, health probes) in new `#console` section, terminal-framed 2-col grid, lazy-load + fixed dimensions, zero external requests | 2026-07-13 | 8c1cd91 | [260712-t3r-add-admin-screenshot-showcase-to-site](./quick/260712-t3r-add-admin-screenshot-showcase-to-site/) |
| 260712-vfl | Click-to-enlarge lightbox with carousel nav for the 4 site screenshots: CSP-safe (static dialog shell + class toggling, site.js append-only), full a11y (focus trap/return, Esc/arrows/wrap-around, mono counter), headless-verified A1-A8 incl. no-JS inertness + 390px mobile | 2026-07-13 | 566f035 | [260712-vfl-screenshot-lightbox](./quick/260712-vfl-screenshot-lightbox/) |
