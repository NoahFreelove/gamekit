# Requirements: GameKit — v1.0 ARCHIVE (all 92 satisfied)

> 📦 **Archived 2026-05-30** on milestone v1.0 completion. All 92 v1 requirements satisfied — see [v1.0-MILESTONE-AUDIT.md](../v1.0-MILESTONE-AUDIT.md) and [v1.0-ROADMAP.md](v1.0-ROADMAP.md). v2 candidates are listed at the bottom; carried-forward tech debt is in the roadmap archive.
...
- [x] **RANK-01**: Library ships as `GameKit.Rankings` NuGet package
- [x] **RANK-02**: `ladders` entity (name, algorithm, is_active, config JSONB)
- [x] **RANK-03**: `player_ranks` entity per ladder with rating, rating_deviation, volatility, wins/losses/draws, last_match_at — rating columns stored as `double precision` (NOT `NUMERIC(8,2)`)
- [x] **RANK-04**: `IRankingAlgorithm.Apply(state, batch)` interface — batched (NOT per-match) to prevent silent Glicko-2 corruption
- [x] **RANK-05**: Default `Glicko2Algorithm` vendored from MaartenStaa/glicko2-csharp (MIT, GPL-compatible)
- [x] **RANK-06**: 1000-match convergence integration test for Glicko-2 implementation
- [x] **RANK-07**: Rank records created lazily on first match (NOT on player registration)
- [x] **RANK-08**: Leaderboard queries: top-N AND around-me, sorted by rating DESC; `(ladder_id, rating DESC)` index hot-path
- [x] **RANK-09**: `AddLadder("name")` registration API
- [x] **RANK-10**: Seasonal leaderboard reset + archival (rating snapshots preserved, current rank reset per season config)
- [x] **RANK-11**: Session-complete endpoint: `POST /api/sessions/{id}/complete` — idempotent via state-conditional UPDATE + cached rating deltas + `Idempotency-Key` header support
...
- [x] **RANK-14**: Per-package migrations targeting `gamekit` schema with `__ef_migrations_rankings` history table
...
- [x] **PRES-01**: Library ships as `GameKit.Presence` NuGet package — Redis-only (no EF entities)
- [x] **PRES-02**: Implements `Core.IPresenceProvider`
- [x] **PRES-03**: Heartbeat endpoint: client posts liveness; expires via Redis TTL
- [x] **PRES-04**: Status states: online / offline / in-match
- [x] **PRES-05**: Abandonment grace period (game-server-authoritative — server reports the abandonment, presence does not infer)
- [x] **PRES-06**: Admin UI presence panel (top-N online, per-player status)
- [x] **OPEN-01**: OpenAPI spec generated by `Microsoft.AspNetCore.OpenApi` covering all GameKit HTTP endpoints
...
- [x] **DIST-02**: Integration test asserts `gamekit_reader` cannot INSERT into `gamekit.sessions`
- [x] **DIST-03**: `SampleGame` reference application using all packages, demonstrating `gamekit_reader` from the game-server side
- [x] **DIST-04**: `GameKit.Template` NuGet template package: `dotnet new gamekit` wraps SampleGame
- [x] **DIST-05**: Production-readiness ops guide (bare-metal, container, air-gapped deployment recipes)
- [x] **DIST-06**: All public APIs have XML doc comments — CS1591 enforced as error across all packages
...
- [x] **OPS-04**: Coordinated SemVer release train: all 6 packages stamp the same MinVer-derived version per release; sibling refs exact-pinned `[X.Y.Z]`
- [x] **OPS-05**: Runtime startup assertion: all GameKit packages report matching `GameKitVersion` constant; fail-fast on mismatch
...
|-------------|-------|--------|
| CORE-01..17 | Phase 1 | ✅ Complete (2026-04-18) |
| OPS-01, OPS-02, OPS-03, OPS-06, OPS-07, OPS-08, OPS-09, OPS-10 | Phase 1 | ✅ Complete (2026-04-18) — OPS-07 satisfied by reflection-based EgressGuardTests (ROADMAP's `unshare --net` CI-layer claim is doc drift, not a gap) |
| DIST-01 (initial: Postgres/Redis + roles) | Phase 1 | ✅ Complete (2026-04-18) |
| AUTH-01..16 | Phase 2 | ✅ Complete (2026-04-18) |
| ADMIN-01..12 | Phase 3 (+3.1) | ✅ Complete 12/12 — anchored by `tests/GameKit.Admin.Integration.Tests/` per plan 03-13 SC-anchor matrix; redesign 3.1 verified 6/6 (2026-05-15) |
| RANK-01..14 | Phase 4 | ✅ Complete (2026-05-16) |
| MATCH-01..15 | Phase 5 | ✅ Complete 15/15 — MATCH-12 chaos (05-09), MATCH-13 load gate (05-10, MaxIterationMs=29 / Dropped=0 / Matched=3092). NOTE: EloRange operates on rating=0 in v1 (no Rankings runtime dep) — rating-aware matchmaking is v1.x backlog |
| PRES-01..06 | Phase 6 | ✅ Complete (2026-05-26) |
| OPEN-01 | Phase 6 | ✅ Complete (2026-05-26) |
| DIST-02..06 | Phase 6 | ✅ Complete (2026-05-26) |
| OPS-04, OPS-05 | Phase 6 (final release-train wiring) | ✅ Complete (2026-05-26) |
